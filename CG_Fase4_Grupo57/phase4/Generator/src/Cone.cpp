...
void Cone::addTopSlice(int slice,int stack,int not_last) {
    Triangle t;
    pair<int,int> point_before(slice-1,stack);
    pair<int,int> point_after(slice,stack);
...
void Cone::addSquareSlice(int slice,int stack) {
    pair<int,int> bottomleft(slice-1,stack),bottomright(slice,stack),topleft(slice-1,stack+1),topright(slice,stack+1);
...
void Cone::addBase(int index) {
    ...
    for(int slice = 0; slice < nSlices; slice++) {
        ...
        texCoords.push_back(make_pair<float,float>(cos(slice_angle_increment*slice) * 0.1875 + 0.8125,sin(slice_angle_increment*slice) * 0.1875 + 0.1875));
...
void Cone::computeNormals(int stack) {
    for(int i = 0; i <= nSlices; i++) {
        pair<int,int> beforepoint((i-1==-1?nSlices:i-1),stack);
        pair<int,int> afterpoint((i==nSlices?0:i+1),stack);
        pair<int,int> actualpoint(i,stack);
        Point3D topPoint;
        if(stack != nStacks-1) {
...
shared_ptr<Model> Cone::generate() {
    ...
    texCoords.push_back(make_pair<float,float>(0.8125,0.1875));
    ...
    for(int i = 0; i < nSlices; i++) {
        ...
        texCoords.push_back(make_pair<float,float>(float(x_tex_coord),1.0f));
    }
    ...
    for (int stack=nStacks-1;stack>=0;stack--) {
        ...
        for (int slice=0;slice<=nSlices;slice++) {
            ...
            texCoords.push_back(make_pair<float,float>(slice*tex_x_increment,stack*tex_y_increment + 0.375));
    ...
    return make_shared<Model>(vertexes,faces,normals,texCoords,height > radiusBase ? height : radiusBase);
}